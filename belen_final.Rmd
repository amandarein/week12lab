---
title: "Decision Tree Lab -- Belen's Version"
...
# Make sure only complete cases are in the data set so that we can properly analyze the data.
cancer <- cancer[complete.cases(cancer), ]

# Collapse some of the variables so that they are better able to be analyzed; otherwise, some variables would have too few of each type
...
# We took our columns that did not have much or any variation, since they do not add any infomration to the model
cancer <- cancer[-c(1, 7,8)]

# We removed rows that had unique values for some of the columns
cancer <- cancer[cancer$HER2.Final.Status != "Equivocal", ]
cancer <- cancer[cancer$AJCC.Stage != "IV", ]

# Make some of the variables into factors, as that will make them easier to work with
cancer$HER2.Final.Status <- as.factor(cancer$HER2.Final.Status)
cancer$PR.Status <- as.factor(cancer$PR.Status)

# Separate out the data into testing and training sets, setting the seed so that we always partition the same way
set.seed(1982)
...
# Find the base rate by dividing the number of positive PR.Status patients by the total number of patients
...
# Build RPART model with the training data set we created
set.seed(1981)
...
# Look at the results of the model, which shows us the most relevant variables for the tree
tree_prstatus
```
...
# Show the tree to see the results in an easily-readible way
...
# Look at the complexity parameters to best understand the number of splits that were used in the tree that was created
...
# Look at the importance of the variables and compare that to where the variables are on the tree

most_imp_vars <- tree_prstatus$variable.importance
most_imp_vars

# Plot the CP to see it visualized to understand the results above
...
# Using the tree we created, use the predict function on our test set to classify the patients for + or - PR.Status
...
# Change tree predict levels to match the levels of the data set
...
# Look at the confusion matrix to see how our results did
...
# Below we calculate the true error rate, which is when the predicted classification is positive when the true classification is not
...
# Below we create the ROC curve to see how our model does compared to the baseline
...
# **** This is done on a previous chunk ****
```
...
# Get our data and environment ready to build the C5.0 tree
...
# Train the C5.0 model using the partitioned data, turning off warnings because we get many but they do not affect our outcome
options(warn = -1)
...
As it can be seen in the confusion matrix below, our results are not looking great, though there was one positive. We never had any false negatives, which is a good thing. This is reflected in our perfect Specificity score. Our accuracy was 50%, which is slightly lower than our base rate. Further, our kappa value is 0, which states zero inter-rater reliability. Our sensitivity is also 0, which means that we have a huge issue with false positives. 

It seems like we are always classifying things as positive for PR.Status. This is not good, as that means there is no nuance or discriminating factors. This means our model is not doing a good job at taking into account how the variables should predict the positive or negative status.
...
# Again, we calculate the error rate
...
# Same steps as above for cleaning the data to prep for the new analysis
...
# Look at the model we create to see the most important variables in the tree
tree_tumor
...
# Use the CP chart to understand how big the optimal tree should be
plotcp(tree_tumor) 
...
```{r, message=FALSE, echo = FALSE, warning=FALSE}
# Use the variable importance to see how each variable contributes to reduction in error provided when it is included
...
# Using the model we created, use the predict function to apply it to the test set
...
```{r, message=FALSE, echo = FALSE, warning=FALSE}
# Use the confusion matrix to compare our results with the base rate, along with looking at other characteristics

tum_conf_matrix = table(tree_predict2, test2$Tumor)
tum_conf_matrix


# Calculating error rates for each type of tumor
...
# Look at the detection rates for each type of tumor
...
# Create an ROC for each type of tumor to see how the model did
...
# Again, creating the testing and training sets
...
# Take out some of the rows that are not allowing the C5.0 analysis to run
...
```{r, message=FALSE, echo = FALSE, warning=FALSE}
# visualize the re-sample distributions
# Look at CP plot to understand whether or not winnowing should be used and how many trials are best
...
```{r, message=FALSE, include = FALSE, warning=FALSE}
# Now, based on the tree we created, prdict the test set
tumor_predict = predict(tumor_mdl, test2, type= "raw")

# Use the confusion matrix to analyze the output of the predict function
...
For the Tumor variable, neither analysis worked particularly well.  Both models were only predicting tumors to be T2, likely because most of the training data was dominated by T2 tumors. In order to improve the models for this variable, we probably need a more robust data set that is more balanced, so that we are able to better understand what factors are related to the variable of interest. Additionally, it would be good if we didn't need to cut out so many of the columns/variables from our analysis, as that would likely improve the accuracy of the model (unless they are not related at all, but we cannot know that unless they are a part of the model). The most unexpected part of this lab is how both models ended up predicting the same thing, and that the C5.0 model was no better than the RCART analysis. We expected C5.0 to be better due to the fact that that type of analysis tends to be better for continuous or multi-class factors, like the tumor variable is.
