...
Looking at the table below, the model only predicts tumors as T2 and T3, so overall the error rate is pretty high (33.33%). Since T2 and T3 tumors have the highest base rates (are most common in the data), this is not too shocking.

T1 - The error rate for T1 is 100% and the detection rate is 0%. There is only 1 T1 tumor in the test set and it is being incorrectly identified as a T2 tumor.

T2 = The error rate for T2 is 14.29% and the detection rate is 66.67%. Compared to the base rate of 61.90%, the model slightly improves the accuracy of predicting a T2 tumor.

T3 - The error rate for T3 is 0% and the detection rate is 14.29%. The model correctly identifies all 3 T3 tumors correctly. Compared to the base rate of 18.10%, this is a big improvement. In terms of the ROC curve, a threshold of 0.49 gives us an ROC that appears to have a slope of 1, but the AUC is 0.7585.

T4 - This type of tumor was not used for training the model due to its low prevalence in the data. Therefore, we are unsurprised that 3 T4 tumors are being incorrectly identified as T3 tumors when we use the model.
```{r, message=FALSE, echo = FALSE, warning=FALSE}
...
##### Evaluate Performance
Due to the complications with prediction, we can only evaluate our tree using 5 columns. As evident in our confusion matrix, the model does not perform very well. Although all the T2 tumors were correctly predicted, all but one of the other tumors were incorrectly classified as a T2 tumor. The accuracy is 66.67%, which isn't terrible, but knowing that only the T2 tumors are being classified correctly, we are not impressed with this number. We have a Kappa value of 0.0667, which is very low. Overall, the C5.0 model we generated to predict the tumor class is not successful, and is much worse in comparison to the CART model. 
...
tumC50CM <- confusionMatrix(as.factor(tumor_predict), as.factor(test2$Tumor), 
                 dnn=c("Prediction", "Actual"), mode = "sens_spec")
